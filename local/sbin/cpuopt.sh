...
ffdecsa_test() {
  ...
  cp $FFdecsaDIR/*.c $FFdecsaDIR/*.h $FFdecsaDIR/Makefile "${TMP_DIR}"
  ...
  for var in ${FFDECSA_MODES}; do
    ...
    make -C "${TMP_DIR}" -e FFdecsa_test "PARALLEL_MODE=${var}" >/dev/null 2>&1
    if test $? -ne 0 ; then
      if [ $_VERB -ge 1 ]; then
      ...
      MAX_M_val=0
      rm -f ${TMPOUT}
      sync;sleep 2; "${TMP_DIR}/FFdecsa_test" > /dev/null 2>"${TMPOUT}"
      if test $? -ne 0; then
        if [ $_VERB -ge 1 ]; then
          echo "    ...failed!"
        fi
        ...
        if test $? -ne 1; then
          ...
          if [ $MTimes -ge 2 ]; then
            for i in `seq 2 $MTimes`; do
              # sync;
              # sleep 1;
              "${TMP_DIR}/FFdecsa_test" > /dev/null 2>"${TMPOUT}"
    ...
    make -C "${TMP_DIR}" clean >/dev/null 2>&1
